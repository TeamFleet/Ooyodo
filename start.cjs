...
const { strPaddingLength, strPaddingStr } = require('./libs/vars.cjs');
...
const run = async () => {
    ...
    await (async () => {
        ...
        const token = await require('./steps/get-token.cjs')(argvs);
        await require('./steps/ensure-directories.cjs')();
        await require('./steps/fetch-api-start2.cjs')(token);

        // if (!isOnlyDownload) {
        await require('./steps/prepare-repositories.cjs')();
        // }

        if (isDistHash) {
            console.log(' ');
            await require('./steps/dist-hash.cjs')();
            return;
        }

        const versionsShipsOld =
            await require('./steps/get-versions-ships.cjs')();
        await require('./steps/download-pics-ships.cjs')();
        await require('./steps/download-pics-equipments.cjs')();

        if (!token) {
            await require('./steps/initialize-database.cjs')();
            await require('./steps/select-pics-ships.cjs')(versionsShipsOld);

            if (!isOnlyDownload) {
                if (isKC2Transition) {
                    await require('./steps/kc2-transition.cjs')();
                }
            }
        }
