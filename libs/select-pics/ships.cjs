...
const { db, pathname, enemyIdStartFrom } = require('../vars.cjs');
    ...
    new Promise(async (resolve, reject) => {
        ...
        // 检查新增
        {
            ...
            if (
                ...
                return reject(
                    'api_start2.json 已损坏，请提供 token 以重新下载'
                );
            }
            ...
            const newShips = list
                .filter((filename) => {
                    ...
                    if (id >= enemyIdStartFrom || id in db.ships) return false;
                ...
                newlist.push({
                    ...
                    files: files
                        .filter(
                            (filename) =>
                                !fs
                                    .lstatSync(path.resolve(dirShip, filename))
                                    .isDirectory()
                        )
                        .map((filename) => path.resolve(dirShip, filename)),
                });
            }
        }
        ...
        for (const ship of ships) {
            ...
            // 确定已有的图片
            const pics = [0].concat(ship._extraIllust || []).map((id) => ({
                id,
                name: (() => {
                    switch (parseInt(id)) {
                        case 0:
                    ...
                    for (const pic of pics) {
                        ...
                        for (const picId of check) {
                            if (pic.id == 0) {
                                ...
                                if (
                                    exillust &&
                                    (!Array.isArray(exillust.exclude) ||
