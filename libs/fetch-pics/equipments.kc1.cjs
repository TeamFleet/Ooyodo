...
const { enemyEquipmentIdStartFrom, pathname } = require('../vars.cjs');
...
module.exports = async (onProgress, proxy) =>
    new Promise(async (resolve, reject) => {
        ...
        let { api_mst_slotitem: slotitem } = apiStart2.api_data;
        ...
        let completeIndex = 0;
        const length = slotitem.length;

        for (const obj of slotitem) {
            const id = parseInt(obj.api_id);
            ...
            await new Promise(async (resolve /*, reject*/) => {
                ...
                for (const type of pics) {
                    if (fail) continue;
                    await getFile(
                        url.parse(
                            `http://203.104.209.23/kcs/resources/image/slotitem/${type}/${theId}.png`
                        ),
                        path.join(dir, `${type}.png`),
                        proxy
                    ).catch((err) => {
                        // isDownloadSuccess = false
                        if (err == 404 || err.message == 404) return;
