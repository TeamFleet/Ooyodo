...
const { enemyIdStartFrom, pathname } = require('../vars.cjs');

const getFile = require('../commons/get-file.cjs');
...
module.exports = async (onProgress, proxy) =>
    new Promise(async (resolve, reject) => {
        ...
        if (!fs.existsSync(fileApiStart2)) return reject('api_start2 不存在');
        ...
        const { api_mst_ship: rawShips, api_mst_shipgraph: shipgraph } =
            apiStart2.api_data;

        const ships = {}; // 舰娘元数据
        const map = {}; // 舰娘和加密URL的对应
        const needUpdate = []; // 需要更新的舰娘
        const picsVersionsNew = {};
        // let shipsCount = 0
        let completeIndex = 0;
     ...
     */
        ...
        for (const id of needUpdate) {
            await new Promise(async (resolve, reject) => {
                ...
                const name =
                    ship.api_name +
                    (id >= enemyIdStartFrom && ship.api_yomi !== '-'
                        ? ship.api_yomi || ''
                        : '');
                ...
                await getFile(url.parse(downloadlink), pathFile, proxy).catch(
                    (err) => {
                ...
                if (!isDownloadSuccess) {
                    ...
                    const dirPicsShipsExtractShip = path.join(
                        dirPicsShipsExtract,
                        '' + id
                    );
                    ...
                    fs.readdirSync(dirPicsShipsExtractShip).forEach((file) => {
                        ...
                        const new_name =
                            '_' +
                            Math.floor(parseInt(parsed['name']) / 2) +
