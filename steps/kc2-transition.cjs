...
const createDatastore = require('../libs/commons/create-datastore.cjs');
const logTitle = require('../libs/commons/log-title.cjs');
const compactNedb = require('../libs/commons/compact-nedb.cjs');
const {
    db: data,
    pathname,
    enemyIdStartFrom,
...
const confirmedSameAsPrev = require('../libs/ships-pic-same-as-prev.cjs');
...
module.exports = async () => {
    ...
    const picVersionsEquipments = await fs.readJson(
        pathname.fetched.versions.equipments
    );
    ...
    if (doDeleteRepo) {
        ...
        await remove(path.resolve(pathname.repoPics, 'dist', 'ships'), [
            '0.jpg',
            '0.webp',
            '0-1.png',
            '0-1.webp',
    ...
    if (doIterateSeries) {
        const stepShipsUpdateSameAsPrev = spinner(
            '更新数据：舰娘图鉴是否和前一个改造版本相同'
        );
        const filesToHash = ['8.png', '9.png'];
        ...
        for (const series of Object.values(data.shipSeries)) {
    ...
    if (doUpdateEquipmentsVersions) {
        const stepEquipmentsUpdatePicVersion =
            spinner('更新数据：装备图鉴版本');
        // console.log(db.ships)
    ...
    if (doCopyPics) {
        ...
        const copy = async (from, dest, filesToCopy) => {
            ...
            for (const id of ids) {
                ...
                for (const filename of filesToCopy) {
                    ...
                    await fs.copyFile(from, path.resolve(dirDest, filename));
                }
            }
        };
        await copy(
        ...
        await copy(
            ...
            ['card.png', 'item_character.png', 'item_on.png', 'item_up.png']
        );
        step.succeed();
    }
};
