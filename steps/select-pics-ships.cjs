...
const { enemyIdStartFrom, db, pathname } = require('../libs/vars.cjs');
const spinner = require('../libs/commons/spinner.cjs');
const readDirIds = require('../libs/commons/read-dir-ids.cjs');
const copyFiles = require('../libs/commons/copy-files.cjs');
const removeSubfolders = require('../libs/commons/remove-subfolders.cjs');
...
module.exports = async (versionsOld = {}) => {
    ...
    const add = ({ type = 'ship-new', id, pics = filesDefault, infos }) => {
        if (type === 'new') type = 'ship-new';
        if (type === 'new-file') type = 'ship-new-file';
        if (type === 'extra') type = 'ship-extra';
    ...
    {
        existIds.forEach((id) => {
            if (!fetchedIds.includes(id)) return;

            const dirFetched = path.resolve(dirFetchedShips, `${id}`);
            const dirExist = path.resolve(dirExistShips, `${id}`);

            const filesNew = fs
                .readdirSync(dirFetched)
                .filter(
                    (filename) =>
                        !fs.existsSync(path.resolve(dirExist, filename))
                );
    ...
    // 对比新下载的文件和该舰娘所有已知图鉴
    {
        ...
        for (const id of updated) {
            ...
            for (const filename of filesDefault) {
                ...
                if (fs.existsSync(fileExist)) md5Exist += await md5(fileExist);
            }
            ...
            for (const cgId of extraCgIds) {
                let md5ExistExtra = '';
                const dirExistExtra = path.resolve(
                    dirExistShipsExtra,
                    `${cgId}`
                );
                for (const filename of filesDefault) {
                    const fileExistExtra = path.resolve(
                        dirExistExtra,
                        filename
                    );
